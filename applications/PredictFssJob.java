...
import com.flytek.urlpattern.tree.applications.StatisticJob.UrlStateMergeMap;
import com.flytek.urlpattern.tree.applications.StatisticJob.UrlStateMergeReduce;
...
public class PredictFssJob {
	private final static String NAME = "TestJob2";
	private static final Logger log = LoggerFactory
			.getLogger(PredictFssJob.class);
	...
	public static Job CreateParseJob(Configuration conf, String inpath,
		...
		try {
			...
			job.setJarByClass(StatisticJob.class);
...
//			AvroJob.setOutputValueSchema(job, Schema.create(Schema.Type.STRING));
	...
	static class UrlStateMergeMap
		...
		final String[] array={"news.3g.cn","m.zongheng.com","wap.eastday.com","m.sohu.com","m.babytree.com","w2h.3g.qq.com","i.ifeng.com",
				   "wap.ifeng.com","wap.zongheng.com","travel.3g.cn","wap.lashou.com","wap.cnki.net","ebook.3g.qq.com","www.wealink.com",
				   "m.fenzhi.com","3g.club.xywy.com"} ;
		@Override
		...
		protected void map(AvroKey<String> key, AvroValue<DownloadPage> value,
			...
			try {
				// 按domain来排序
				Random random=new Random();//news.3g.cn m.zongheng.com wap.eastday.com m.sohu.com w2h.3g.qq.com i.ifeng.com
				int num=random.nextInt(80)+10;//wap.zongheng.com travel.3g.cn wap.lashou.com wap.cnki.net ebook.3g.qq.com
				if(flag)
					context.write(new AvroKey<String>(num+host),new AvroValue<String>(url));
				else
					context.write(new AvroKey<String>(host),new AvroValue<String>(url));
	...
	static class UrlStateMergeReduce
		...
		final PageTypeFixedPipeline judgeTool=new PageTypeFixedPipeline();
		@Override
		protected void setup(Context context) throws IOException,
				InterruptedException {
		...
		protected void reduce(AvroKey<String> _key,
			...
			for(AvroValue<String> item:values){
				...
				str=new String[2];
				if(host.startsWith("1")||host.startsWith("2")||host.startsWith("3")||host.startsWith("4")||
					host.startsWith("5")||host.startsWith("6")||host.startsWith("7")||host.startsWith("8")||host.startsWith("9"))
					str[0]=host.substring(2);
				else {
					str[0]=host;
				}
				str[1]=url;
				flag=judgeTool.IsThemePage(str);
				if(flag.equals("THEME_PAGE")){
					sum1++;
				}
//				else{
//					log.info(url+" "+host+" "+PageTypeFixedPipeline.map.get(new Utf8(host)));
//				}
			}
			context.write(new Text(host),new Text(sum0+" "+sum1));
		}
	}
}
